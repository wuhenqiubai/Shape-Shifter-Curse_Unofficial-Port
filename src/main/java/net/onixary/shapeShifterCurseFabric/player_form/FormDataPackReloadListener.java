...
public class FormDataPackReloadListener implements SimpleSynchronousResourceReloadListener {
    ...
    public Identifier getFabricId() {
        return Identifier.fromNamespaceAndPath(ShapeShifterCurseFabric.MOD_ID, "ssc_form");
    }
    ...
    public void onResourceManagerReload(ResourceManager manager) {
        ...
        manager.listResources("ssc_form", identifier -> identifier.getPath().endsWith(".json")).forEach((identifier, resource) -> {
            ...
            Identifier formID = Identifier.fromNamespaceAndPath(identifier.getNamespace(), identifier.getPath().replace(".json", "").substring(9));
            JsonObject formData;
            try {
        ...
        manager.listResources("origins_power_extra", identifier -> identifier.getPath().endsWith(".json")).forEach((identifier, resource) -> {
            Identifier ID = Identifier.fromNamespaceAndPath(identifier.getNamespace(), identifier.getPath().replace(".json", "").substring(20));
