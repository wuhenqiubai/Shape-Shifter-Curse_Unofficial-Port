...
public class ClientConfig implements ConfigData {
    ...
    @ConfigEntry.Category("General")
    @Comment("Ignore noRenderArm power. Default: false")
    ...
    @ConfigEntry.Category("General")
    @Comment("Use bigger(2x) Book of Shapeshifter interface. Default: false")
    public boolean newStartBookForBiggerScreen = false;  // 菜单缩放至少为4

    @ConfigEntry.Category("General")
    @Comment("Auto-modify FPM config. Default: true")
    public boolean enableChangeFPMConfig = true;  // 启用修改FPM配置

    @ConfigEntry.Category("General")
    @Comment("Disable unlock check in form color selection menu. Default: false")
    public boolean disableUnlockCheckInFormColorSelectMenu = false;  // 禁用颜色选择菜单中的解锁检查

    @ConfigEntry.Category("General")
    @Comment("Use the professional version of the form color selection menu. Default: false")
    ...
    @Comment("Unlock all form for form color selection menu. Default: false")
    public boolean unlockAllFormInFormColorSelectMenu = false;  // 解锁颜色选择菜单中的所有形态

    @ConfigEntry.Category("Tool")
    @Comment("Clear form unlock record for form color selection menu. Default: false")
